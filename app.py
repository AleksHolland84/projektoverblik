...
if authentication_status and username == "admin":
    ...
    st.caption('Hvis password input undlades vil password være "password"')
...
if authentication_status and username != "admin":
    ...
    for day in week_day_list:
        ...
        date = log_book(username=username, label="Logbog for {day}", value=logbog.get(f'logbog for {day}'), ref=f"/content_container/usernames/{username}/logbog", toast=f"Logbog for {day} uploaded!")
        if date:
            pass
            #print(date)
